...
class ToTensor:
    ...
    def __call__(self, sample):
        if self.mode == 'train':
            ...
            mask = np.transpose(mask, axes=[3, 0, 1, 2])[0] # whm
            img = torch.from_numpy(img).float()
            mask = torch.from_numpy(mask).long()
            sample['image'], sample['label'] = img, mask
...
class Mirroring:
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            ...
            sample['image'], sample['label'] = img.copy(), mask.copy()
...
class ExtractPatch:
    ...
    def __call__(self, sample):
        ...
        sample['image'] = patch_img
        sample['label'] = patch_mask

        return sample

class RandomCrop:
    """Randomly extracts a patch of a given size from an image (4D numpy array)."""
